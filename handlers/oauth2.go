...
func Token(w http.ResponseWriter, r *http.Request) {
	...
	if grant_type == "authorization_code" {
		...
		if appClient {
			responsePayload["refresh_token"] = r_tok
		} else {
		...
		responses.OK(w, responsePayload)
...
func Refresh(w http.ResponseWriter, r *http.Request) {
	...
	if rawToken == "" {
		...
		if err != nil {
			responses.Unauthorized(w, "missing_refresh_token", "Refresh token is missing")
			return
		}
	...
	if err != nil || !token.Valid {
		responses.Unauthorized(w, "invalid_token", "Refresh token is invalid")
		return
	}
	...
	if !strings.Contains(aud, authDomain) {
	...
	err = collection.FindOne(ctx, bson.M{"family_jti": familyJTI}).Decode(&registeredToken)
	if err != nil {
	...
	dpopClaims, err := jwts.ValidateDPoPWithKnownKey(dpopHeader, r.Method, expectedHTU, ecdsaPubKey)
	if err != nil {
		_, _ = collection.UpdateOne(ctx, bson.M{"_id": tokenID}, bson.M{"$set": bson.M{"banned": true}})
	...
	if _, err = collection.UpdateOne(ctx, bson.M{"_id": tokenID}, bson.M{"$push": bson.M{"DPoP_jtis": dpopJTI}}); err != nil {
		responses.ServerError(w)
		return
	}
	...
	responsePayload := map[string]any{"access_token": at_tok, "token_type": "DPoP", "id_token": id_tok,

		"expires_in": 600}
	if appClient {
		responsePayload["refresh_token"] = rawToken
	}
	responses.OK(w, responsePayload)
}
